...
macro_rules! static_regex {
    ($expr:expr) => {{
        ...
        REGEX.get_or_init(|| Regex::new($expr).unwrap())
    }};
}
...
impl<'a> BytesBuf<'a> {
    ...
    pub fn of<T: Into<Cow<'a, [u8]>>>(elements: impl IntoIterator<Item = T>) -> Self {
        Self {
            buf: elements.into_iter().map(T::into).collect(),
        }
    }
...
impl<'a, T> std::iter::Extend<T> for BytesBuf<'a>
where
    T: Into<Cow<'a, [u8]>>,
{
...
impl<'a, T> std::iter::FromIterator<T> for BytesBuf<'a>
where
    T: Into<Cow<'a, [u8]>>,
{
    fn from_iter<I: IntoIterator<Item = T>>(iter: I) -> Self {
        Self {
            buf: iter.into_iter().map(T::into).collect(),
        }
    }
}
